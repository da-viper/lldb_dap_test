...
"""
    ...
    def build(self, filename=None):
        ...
        shared_lib_name = (
            "libunlikely.so" if sys.platform == "linux" else "libunlikely.dylib"
        )
        self.run_command(
            [
                "/usr/bin/clang",
        ...
        if codesign := shutil.which("codesign"):
            self.run_command([codesign, "-fs", "-", self.getBuildArtifact("a.out")])
    ...
    def test_breakpoint_events(self):
        ...
        - Sets a breakpoint in a shared library from preRunCommands.
        - Sets a line breakpoint in the main executable and a function
          breakpoint on `foo` (defined in the shared library) via DAP.
