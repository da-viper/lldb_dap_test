...
class TestDAP_stackTraceDisassemblyDisplay(DAPTestCaseBase):
    TEST_PROGRAM = r"""int no_source_func(int n);
...
"""
    ...
    def build(self, filename=None):
        main_path = self.create_file(self.TEST_PROGRAM, "main.c")
        other_path = self.getBuildArtifact("other.c")
        self.run_command(
            [
                "/usr/bin/clang",
                "-g",
    ...
    def stop_at_other_c_breakpoint(self):
    ...
    def verify_frames_source(
        ...
        source_0_path = self.expect_not_none(source_0.path)
        source_1_path = self.expect_not_none(source_1.path)
    ...
    def test_stopDisassemblyDisplay(self):
        ...
        session, thread_ctx = self.stop_at_other_c_breakpoint()
