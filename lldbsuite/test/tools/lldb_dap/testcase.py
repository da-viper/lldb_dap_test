...
from lldbsuite.test.tools.lldb_server import lldbgdbserverutils
...
class DAPTestCaseBase(Base, metaclass=LLDBTestCaseFactory):
    ...
    DEFAULT_TIMEOUT: Final[float] = 500.0 if "ASAN_OPTIONS" in os.environ else 30.0
    ...
    def create_debug_adapter(
        ...
        def cleanup_adapter():
            if adapter.is_alive:
                self.logger.info("killing debug adapter")
                adapter.kill()
            if stderr := adapter.process.stderr:
                err = stderr.read()
                self.logger.info("adapter stderr: \n\t%s", err.decode())
